...
3. **Emit the generated body block** (§3) — projects, work, skills, (optional) awards — as a single fenced ```latex``` snippet, with explicit instructions:
   > Replace the template's **fictional example project block** under the "项目与实习经历" / "Projects & Internship Experience" section with the block below. In `templates/latex/resume-cn.tex` this block is marked by the comment `% 项目与实习经历（下方为【虚构示例】，请整体替换为你自己的经历）` (under `\section{...项目与实习经历}`); in `templates/latex/resume-na.tex` it is marked by `% Projects & Internship Experience (the entries below are [FICTIONAL EXAMPLES] — replace with your own)` (under `\section{...Projects \& Internship Experience}`). Paste the generated block over that entire example block. Do **not** modify the preamble or any `\newcommand` / `\renewcommand` definition.
